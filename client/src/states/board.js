...
Board.Projector = class extends Layer {
  ...
  onEvent(event) {
    if(event.isKeyboardEvent()){
      ...
      switch(event.type) {
        case Message.Type.MOUSE_MOVE: Client.emit("mouse_position", event.data);break;
      }
    }
...
Board.SettingsPopup = class extends Popup {
  ...
  _createMatchPanel() {
    ...
    this.matchPanel
      ...
      .addForwarder(Message.Type.MOUSE_CLICK, () => {console.log("Make medic")});
      
    this.matchPanel
      .add(new Button(new Rect(0, 0, 64, 0), "Jacinta"), {fill: [0., 1.]})
      .addForwarder(Message.Type.MOUSE_CLICK, () => {console.log("Make Jacinta")});
    return this;
  }
